...
from crawlix.db.models import Page, PageLink
...
def inbound_internal_counts(session: Session, project_id: int, url_norms: list[str]) -> dict[str, int]:
    if not url_norms:
        return {}
    rows = (
        session.query(PageLink.to_url_norm, func.count(PageLink.id))
        .join(Page, PageLink.from_page_id == Page.id)
        .filter(Page.project_id == project_id, PageLink.to_url_norm.in_(url_norms))
...
def outbound_internal_counts(session: Session, project_id: int, pages_by_id: dict[int, str]) -> dict[int, int]:
    """Counts PageLink rows from each page to same-host targets."""
    if not pages_by_id:
        return {}
    ...
    rows = (
        ...
        .join(Page, PageLink.from_page_id == Page.id)
        .filter(Page.project_id == project_id, PageLink.from_page_id.in_(ids))
        .all()
    )
