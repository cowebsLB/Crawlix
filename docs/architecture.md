...
|--------|----------------|
| **UI (PyQt6)** | `src/crawlix/ui/` — pages, shell, wizard, `project_dialog.py` (J3); **Keywords** page tables + SERP picker + Matplotlib rank chart (J6–J7); **Citations** page tabs for built-in sources, locations, check history + CSV export (J8 foundation); calls services via workers / facades; **no raw SQL in widgets** (current code uses session in main window for MVP — extract facades as the app grows). |
...
| **Services** | `src/crawlix/services/` — crawler BFS (`run_crawl_job` supports optional **`on_progress`** callback), audit HTML, SERP placeholder fetch, citations seed (`seed_builtin_sources`), updater, integrations stubs, **`exporters`** (CSV/JSON for pages, links, audits — J4/J5; built-in citation sources CSV — J8). |
